...
async def store_oauth_code(
    ...
    try:
        ...
        token_data = await get_access_token(auth_code=gmail_req.code,
        ...
        await save_gmail_token_to_data_source(data_source_service, gmail_req.state, token_data, user_email, redirect_uri)
        logger.info(f"Gmail tokens saved successfully")

        return GmailAuthResponse(
            status="success",
            message="Gmail authentication successful. Tokens saved."
        )
...
async def save_gmail_token_to_data_source(
    ...
    try:
        ...
        logger.info(
            "Gmail tokens successfully saved to data connection settings")
